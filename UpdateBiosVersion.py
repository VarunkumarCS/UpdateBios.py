import argparse, getpass, json, os, platform, time, subprocess ,logging, requests, sys, warnings
...
def script_examples():
    print("""\n- GetSetPowerStateREDFISH.py -ip 192.168.0.120 -u root -p calvin --get, This Script will return current server firmware and bios version.""")
    sys.exit(0)

#HOW TO RUN IN THE WINDOWS
#python UpdateBiosVersion.py -ip 10.2.161.103 -u root -p calvin --location C:\Users\vKumar\code\Dell --image BIOS_76NC0_WN64_2.13.3.EXE
#python UpdateBiosVersion.py -ip 10.2.161.103 -u root -p calvin --location C:\Users\vKumar\code\Dell --image BIOS_76NC0_WN64_2.13.3.EXE --force
#python UpdateBios.py -ip 10.2.161.103 -u root -p calvin --protocol HTTP --uri http://10.2.255.252/tftpboot/DELL_ICP/BIOS_76NC0_WN64_2.13.3.EXE --reboot
...
def get_current_bios_version():
    ...
    data = response.json()
    if response.status_code != 200:
        logging.warning("\n- WARNING, GET request failed to get current servers's bios version, status code %s returned." % response.status_code)
        logging.warning(data)
        sys.exit(0)
    logging.info("\nServer %s Bios Version: %s\n" % (idrac_ip, data['BiosVersion']))
